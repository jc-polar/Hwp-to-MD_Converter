...
### 🛡️ Windows PC 보호(SmartScreen) 안내
...
- **📄 한글 문서(HWP/HWPX) 완벽 대응**: HWP, HWPX를 비롯해 PDF, Word, Excel 등 실무에서 자주 쓰이는 다양한 문서를 지원하며 문서 서식과 수치 정합성을 보존합니다.
- **🛡️ 프롬프트 인젝션 & 은닉문자 7대 조건 사전 멸균 (`CleanPromptInjection`)**:
  - 유니코드 제어문자(ZWC/FFFD), 0pt 극소 폰트, 장평 0%, 흰 배경에 흰색 글씨(White-on-White), 한글 [숨김] 속성 내용을 탐지 및 삭제합니다.
...
1. **폴더/파일 선택**: 변환할 문서를 선택하거나 앱 화면으로 드래그 앤 드롭합니다.
2. **변환 형식 설정**:
...
3. **옵션 설정**:
   ...
   - `G-Notebook 자동 업로드`: 대상 노트북의 URL 주소를 입력하면, 문서 변환 후 G-Notebook(구 NotebookLM) 업로드가 자동 진행됩니다.
...
- **Frontend**: HTML5, Vanilla JavaScript, CSS3 (`Pretendard` Font, Sleek Dark Mode & Glassmorphic UI)
- **Backend (Desktop Framework)**: [Tauri v2](https://tauri.app/) (Rust 기반 멀티스레드 및 Single-Instance 관리)
...
[사용자 드래그 앤 드롭 / 폴더 선택]
        ↓
[main.js] ── 파일 목록 수집 ──→ [Rust: process_dual_documents]
                                        ↓
                         ┌──────────────┴──────────────┐
                         ↓                             ↓
          [node.exe + worker_bundle.cjs]      [pdf_worker.exe (C# OLE)]
          HWP/HWPX → MD (RAG/2D Grid 정제)     HWP/HWPX → PDF (N-Key 우회 & 멸균)
                         └──────────────┬──────────────┘
                                        ↓
                  [Gemini Notebook (구 NotebookLM) 3원 파도타기 업로드]
```
